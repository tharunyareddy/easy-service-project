...
public class ContractorServiceImpl implements IContractorService {
	...
	public List<Maintenance> getAllMaintenance() throws MaintenanceNotFoundException{
		...
		ResponseEntity<List> response =restTemplate.getForEntity(url,List.class);
		if(response.getBody().isEmpty()) {
			throw new MaintenanceNotFoundException("No Maintenanaces");
		}
		return response.getBody();
	}
	...
	public List<Contractor> getContractNameByMaintenanceManager(String maintenanceManager)
			throws ContractNotFoundException {
		String url=BASEURL+"/maintenanceManager/"+maintenanceManager;
		List<Contractor> maintenanceList = contractorRepository.findContractNameByMaintenanceManager(maintenanceManager);
